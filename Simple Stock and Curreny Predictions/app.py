...
ticker_symbol = st.text_input("Enter Stock Ticker or Currency Pair (e.g., 'AAPL' or 'USDPKR=X')")
...
if st.button("Predict") and ticker_symbol:
    if asset_type == "Stock":
        ...
        history = yf.download(ticker_symbol, interval="1d", start="1900-01-01", end=str(datetime.now().date()))
    ...
    st.write(pd.DataFrame(reg.predict(X_test), columns=['Close', 'High', 'Low', 'Open']))
    ...
    future_dates = pd.to_datetime([features_df.iloc[-1]['dates'] + pd.Timedelta(days=x) for x in range(1, 101)])
    ...
    predict_df = pd.DataFrame(future_preds, columns=['Close', 'High', 'Low', 'Open'], index=future_dates)
